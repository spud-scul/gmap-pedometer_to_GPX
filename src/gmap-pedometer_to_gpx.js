/*
  gmap-pedometer_to_GPX by Spud for SCUL mission exports, tested only with
  https://www.gmap-pedometer.com/ and with SCUL mission prompts for filename
  and track name. See
  https://github.com/spud-scul/gmap-pedometer_to_GPX
...
function gmaptogpxdiv(dtype) { 
    ...
    */
	...
	/* FIXME 
	   errorbox('<p>2008.05.07 - gmap-pedometer_to_GPX is currently unable to process driving directions, due to a change in the underlying Google Maps code. I\'m working on it.</p>');
	   closebox();
	   return(0);
	   END FIXME */
...
';
    ...
    t+='<ul class="menubar">';
    t+='<li class="menubar">gmap-pedometer_to_GPX v' + version + '</li>';
    if ((qtype > 1) && (qtype != 4)) {
	if (dtype != "route") {
	...
	let operation = prompt("Enter the name of the operation:", "Operation XXX");
	let date = prompt("Enter the stardate of the mission:", "000000")

    xmlt = ''
	
	/* This part of the GPX is going to be the same no matter what. */
    xmlt+= '<?xml version="1.0" encoding="' + charset + '" ?>\n' + 
	'<gpx version="1.1"\n' + 
	'     creator="gmap-pedometer_to_GPX' + version + ' - https://github.com/spud-scul/gmap-pedometer_to_GPX"\n' + 
	...
	xmlt+= '   <trk>\n';
	xmlt+= '      <name>Google Driving Directions Track</name>\n';
	...
	for(var i=0;i<gLatLngArray.length;i++){
	    ...
	    xmlt+= 'lat="' + round(gLatLngArray[i].lat) + '" ' +
		...
		currentElevation = round(currentElevation * 0.3048)     
		    xmlt+= '         <ele>' + currentElevation  + '</ele>\n';
	    }

	    xmlt+= '         <name>' + (i ? 'Turn ' + i : 'Start') + '</name>\n';
	...
	for (var i = 0; i < lyr.length; i++) {
	    xmlt+= '   <trkpt lat="' + lyr[i].lat + '" lon="' + lyr[i].lon + '">\n' ;
	    xmlt+= '      <name>LYR' + i + '</name>\n' + '   </trkpt>\n';
	}
	t += '  </trkseg></trk>\n';
	...
	for (var key in routes) {
		...
		xmlt+= '      <name>' + key + '</name>\n';
		xmlt+= '      <cmt>' + milestones[routeno] + " to " + milestones[routeno + 1] + '</cmt>\n'; 
		xmlt+= '      <desc>' + milestones[routeno] + " to " + milestones[routeno + 1] + '</desc>\n'; 
	    }
	...
	for(i=0;i<routes['poi'].length;i++){
	    ...
	    xmlt+= '   <wpt lat="' + point.lat + '" lon="' + point.lon + '">\n' +
	...
	errorbox('An unknown error occurred. Please leave a bug report at the <a href="https://github.com/spud-scul/gmap-pedometer_to_GPX">project homepage</a> and include a link to the page you\'re on right now.');
	error = 1;
    }
	...
	let filename = 'SCUL_' + date + '_' + operation + '.gpx';
	filename = filename.replace(/ /g,"_");// Spaces to underscores
	filename = filename.replace(/[ &\/\\#,+()$~%.'":*?<>{}]/g, "");// Eliminate special characters

	filename = 'SCUL_' + date + '_' + operation;
	filename = filename.replace(/ /g,"_");// Spaces to underscores
	filename = filename.replace(/[ &\/\\#,+()$~%.'":*?<>{}]/g, "_");// Eliminate special characters
	filename += '.gpx'

	download(filename, xmlt);
...
function loadabout() {
    var about = '<span style="font-size: x-small;"><p><a href="https://github.com/spud-scul/gmap-pedometer_to_GPX">gmap-pedometer_to_GPX</a> Extractor ' + version + '<br />';
    about += '<p>Usage:<ul>';
    ...
    about += '</ul>If you have questions or comments, please visit the <a href="https://github.com/spud-scul/gmap-pedometer_to_GPX">project homepage</a>.</p></span>';
    showstatusdiv(about);
}

function errorbox(a) {
    var err = '<a href="https://github.com/spud-scul/gmap-pedometer_to_GPX">gmap-pedometer_to_GPX</a> v' + version + " (ERROR)<br />" + a;
    showstatusdiv(err);
}
